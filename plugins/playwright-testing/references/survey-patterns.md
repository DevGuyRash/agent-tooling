...
`renderGallery(runDir)` produces a portable local index. Captures open first with one readable preview per observation and frame controls for sequences. Flow uses each image’s proportions and places the next card in the available column; Contact sheet uses uniform previews for scanning. The layout choice is remembered for that evidence run when browser storage is available. Native PNG/JPEG dimensions reserve a sequence’s largest frame within its own card. Supporting records have a compact view; search offers matches in either view, and original identities and context remain available in each record. Collections containing only supporting records open that view directly. `readEvidenceRun(runDir, options)` returns a compact summary and a bounded record page; choose `type`, `taskId`, `id`, `query`, `offset`, `limit`, or `fields` for the material needed. The JavaScript API accepts `limit: Infinity` for an explicit full read. Retain originals when extracting a smaller review set.
