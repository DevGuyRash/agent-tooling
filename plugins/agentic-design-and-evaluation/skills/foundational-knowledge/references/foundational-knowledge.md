...
```mermaid
flowchart TB
  classDef knowledge fill:#eef2ff,stroke:#64748b,color:#0f172a
  classDef boundary fill:#fff7e6,stroke:#887744,color:#111111
  classDef judgment fill:#edf8ef,stroke:#447755,color:#111111

  K["Knowledge worth retaining<br>Principles, observations, examples, hypotheses"]:::knowledge
  D["Authoring judgment<br>What this task, recipient, and environment need"]:::judgment
  C["Information worth communicating<br>Facts, preferences, proposals, examples, requirements"]:::knowledge
  R["Useful restraint<br>Omit unnecessary coaching and premature solution choices"]:::judgment
  A["Legitimate authority and scope<br>Who may decide and commit to what"]:::boundary
  B["Applicable obligations and permissions<br>Depend on actual authority and scope"]:::boundary
  P["Provisional interpretations<br>Remain available for assessment"]:::knowledge
  V["Evidence and relevant reality<br>Ground factual confidence and feasibility"]:::judgment
  I["Interpreted communication<br>Meaning, force, examples, context, presentation"]:::knowledge
  F["Recipient and surrounding arrangement<br>Existing behavior, sources, tools, resources,<br>state, interfaces, and interaction"]:::judgment
  E["Executor judgment within the mandate<br>Can include discovering the useful contribution"]:::judgment
  O["Observed response or action<br>May or may not satisfy the legitimate task"]:::knowledge
  Q["Assessment of contribution<br>Usefulness, correctness, boundaries,<br>support, correction, and total cost"]:::judgment

  K -->|can inform| D
  D -->|when helpful| C
  D -->|can also support| R
  A -->|can establish| B
  A -->|can be conveyed through| C
  V -->|can be conveyed through| C
  C -->|does not automatically become| B
  C -->|may contain| P
  C -->|is interpreted as| I
  I -->|can influence without authorizing| E
  V -->|does not itself grant| A
  V -->|can support or undermine| P
  V -->|informs| E
  B -->|bounds within its scope| E
  P -->|informs without predetermining| E
  E -->|can revise| P
  R -->|can preserve discretion for| E
  F -->|enables or limits useful action by| E
  F -->|also shapes| O
  E -->|contributes to| O
  O -->|does not establish its own quality| Q
  B -->|grounds applicable requirements for| Q
  V -->|supports| Q
  Q -->|can revise| K
```
...
```mermaid
flowchart TD
  R["Selected representation<br>Prose, table, diagram, schema, or another view"]
  H["Possible enabling effects<br>Expose relationships, support operations,<br>make checks possible, reduce avoidable work"]
  L["Possible limiting effects<br>Hide distinctions, imply false completeness,<br>make useful findings hard to express"]
  C["Contribution in this setting<br>Depends on the question, recipient, evidence,<br>interface, and available revision routes"]
  T["No universal ranking<br>Earlier or later, shorter or longer,<br>structured or open can each be appropriate"]

  R -->|can enable| H
  R -->|can restrict| L
  H -->|may help| C
  L -->|may hinder| C
  C -->|requires a contextual judgment| T
```
...
```mermaid
flowchart TB
  classDef candidate fill:#fff7e6,stroke:#887744,color:#111111
  classDef evidence fill:#edf8ef,stroke:#447755,color:#111111

  Insight["Conceptual insight, reported failure, or material risk"]:::candidate
  Intervention["Candidate support<br>Wording, source access, representation,<br>memory, controls, or evaluation"]:::candidate
  Baseline["Relevant comparison<br>Keep non-target factors comparable<br>Identify intended changes explicitly"]:::evidence
  Comparison["Observed outcomes<br>Quality, interface compliance, useful correction,<br>needless intervention, boundary violations,<br>total effort, and relevant uncertainty"]:::evidence
  Effect["Scoped finding<br>An effect is not automatically a benefit<br>A benefit is not proof of a mechanism"]:::evidence
  Explanation["Candidate causal explanation<br>Needs discriminating evidence<br>beyond a plausible story"]:::candidate
  Decision["Retain, revise, remove, or investigate<br>According to purpose, grounds, and tested scope"]:::candidate
  Obligation["Separate question<br>An underlying obligation can apply<br>without a useful extra reminder"]:::boundary

  Insight -->|suggests but does not validate| Intervention
  Intervention --> Comparison
  Baseline --> Comparison
  Comparison --> Effect
  Effect -->|does not uniquely establish| Explanation
  Explanation -->|can motivate another comparison| Comparison
  Effect -->|can inform| Decision
  Decision -->|changing conditions can justify| Comparison
  Obligation -->|limits inferences about legitimacy from| Effect

  classDef boundary fill:#eef2ff,stroke:#64748b,color:#0f172a
```
...
```mermaid
sequenceDiagram
  actor User as User or legitimate decision owner
  participant Author as Author or coordinator
  participant Expert as Executor or consulted model
  ...
  opt Investigation would help and is authorized
    Expert->>Sources: Obtain needed information within existing authority
    Sources-->>Expert: Material with identity, provenance, and limitations
    opt A bounded probe is useful and authorized
      Expert->>Environment: Conduct the probe
      Environment-->>Expert: Observations, results, failures, or unresolved gaps
    end
  end

  Expert-->>Author: Useful contribution, possibly a correction, better question, or no new artifact

  opt New evidence or substantive disagreement
    Author->>Expert: Material information or a genuine challenge, not cosmetic normalization
    Expert-->>Author: Revised or defended contribution with relevant support
  end

  opt Correction or further action is warranted
    alt Action is within the executor's mandate
      Expert->>Environment: Execute or investigate as warranted
      Environment-->>Expert: Actual result, failure, or unresolved state
    else Another decision owner or additional authority is needed
      Expert-->>Author: Concrete issue, consequence, evidence, and decision boundary
      opt Decision is also outside the coordinator's mandate
        Author->>User: Present the consequential choice
        User-->>Author: Retain, revise, approve, or decline
      end
      Author->>Expert: Updated assignment or delegation, when applicable
    end
  end

  Expert-->>Author: Actual findings, usable result, completion status, and uncertainty
  Author-->>User: Preserve the contribution and consequential decision grounds
  Note over Author,Expert: Narrow roles are legitimate. Important findings need a usable route to the responsible decision.
```
...
sequenceDiagram
  ...
  alt Different positions inside one input
    ...
    Note over Model: Both are available before the response. Position does not establish internal timing.
    Model-->>Caller: Response under the supplied information
  else Actual staged availability
    Caller->>Model: Task and all currently necessary requirements, without an optional schema
    Model-->>Caller: Initial contribution
    Caller->>Model: Optional schema and any legitimate new information
    Model-->>Caller: Further contribution or warranted revision
    Note over Caller,Model: Information history, calls, artifacts, resources, and revision opportunities can differ.
  end
```
...
```mermaid
stateDiagram-v2
  state "Working understanding" as Working
  state "Authorized investigation" as Investigation
  state "Supported current commitment" as Commitment
  state "Execution within mandate" as Execution
  state "New evidence or changed conditions" as Evidence
  state "Complete for the current assignment and evidence horizon" as Complete
  state "No useful authorized continuation currently available" as Blocked
  state "Proposed intervention retired or superseded" as Retired

  [*] --> Working
  Working --> Investigation: Work can resolve consequential uncertainty
  Working --> Commitment: Grounds and authority suffice for this commitment
  Investigation --> Evidence: Observe results rather than assume them
  Evidence --> Working: Revise a premise, representation, or direction
  Evidence --> Commitment: Grounds support the next commitment
  Commitment --> Execution: Authority and operational conditions permit
  Execution --> Evidence: Outcomes change what is known
  Execution --> Complete: Evidence supports actual completion requirements
  Evidence --> Complete: The investigation's question is resolved
  Working --> Blocked: No useful authorized continuation is available
  Blocked --> Working: Relevant information, authority, or viable options become available
  Working --> Retired: Proposed work is unnecessary, invalid, or superseded
  Retired --> Complete: Retiring the proposal resolves the current assignment
  Retired --> Working: An unresolved purpose or new material grounds warrant further work
  Complete --> Working: Later material grounds warrant reopening

  note right of Investigation
    Diagnosis does not require a replacement solution.
    A local defect need not reopen the whole assignment.
    Reversibility does not create permission.
  end note

  note right of Complete
    Further possible work does not erase bounded completion.
    Recorded reopening cues are not an exhaustive whitelist.
    Revising a belief does not undo every external consequence.
  end note
```
...
erDiagram
  ...
  DECISION {
    ...
    string rationale
    string dependencies
    string reopening_cues
  }
  EVIDENCE {
    string observation_or_derived_result
    string uncertainty
    string revision
  }
  SOURCE {
    string identity
    string accessible_location
    string access_boundary
  }
  ACTIVE_VIEW {
    string current_objective
    string live_commitments
    string unresolved_issues
  }

  AUTHORITY }o--o{ DECISION : bounds
  SOURCE }o--o{ EVIDENCE : supports_traceability
  EVIDENCE }o--o{ DECISION : supports_or_undermines
...
```mermaid
mindmap
  root((Collaboration with much stronger intelligence))
    Distinctions worth preserving
      Knowledge communication and obligation
      Meaning interpretation effect and explanation
      Authority credibility and influence
      Permission and practical opportunity
      Working commitment and external consequence
      Evidence derivation and independent observation
      Coverage correctness and adequacy
    Conditional enduring needs
      Relevant reality not already available
      Purposes and developing preferences
      Shared meaning at consequential interfaces
      Legitimate decision rights
      Useful action and responsible correction
      Recoverable decision grounds
      Evidence sufficient for intended reliance
    Replaceable implementations
      Particular wording and modal conventions
      Fixed headings and answer templates
      Skill routing and storage layouts
      Diagrams and decision record formats
      Specific approval and testing mechanisms
    Empirical questions
      Which support helps which task
      How compression preserves meaning
      How representations enable discovery
      What makes a correction actionable
      Which effects survive model changes
      Whether evaluation measures useful work
      How stronger results become assessable
    Limits on the foundation itself
      No extra coaching is a baseline rather than a doctrine
      A capability multiplier is a thought experiment
      Nonbinding knowledge can still influence
      Agreement is not independent validation
      A useful method can have a mistaken explanation
      No universal optimum is established
```
...
- <details><summary>🧬 Foundational knowledge</summary>
  <p>Explanatory distinctions, arguments, and scoped observations that can inform future research and design without necessarily becoming universal runtime instructions.</p>
  </details>

- <details><summary>🤝 Delegation</summary>
  <p>An allocation of responsibility and discretion, including which purposes, judgments, actions, or consequential commitments a participant may pursue. It can include discovering the useful objective or contribution rather than only executing a specified artifact.</p>
  </details>

- <details><summary>🎯 Mandate</summary>
  <p>The authorized purposes, work, decisions, changes, and external effects within a participant's role.</p>
  </details>

- <details><summary>🧭 Frame</summary>
  <p>A working interpretation of the problem, what matters, and which questions or contributions appear relevant. It need not be exhaustive or permanent.</p>
  </details>

- <details><summary>🔒 Accidental closure</summary>
  <p>Treating a provisional interpretation, assumption, list, or representation as exhaustive or binding without sufficient grounds. It is one possible mechanism of frame lock, not a complete explanation of every missed alternative.</p>
  </details>

- <details><summary>⚖️ Authority</summary>
  <p>Legitimate standing to direct work, make decisions, or permit commitments within a scope. It differs from technical access, confidence, capability, credibility, and observed influence.</p>
  </details>

- <details><summary>🔎 Credibility</summary>
  <p>Evidential grounds for accepting a claim. Credibility does not by itself authorize action, and an authorized requirement does not prove its factual premises.</p>
  </details>

- <details><summary>🎛️ Steering intervention</summary>
  <p>A specified change to communication or the surrounding arrangement intended to influence behavior. The term does not presume a known internal actuator, established mechanism, or demonstrated benefit.</p>
  </details>

- <details><summary>🧲 Behavioral influence</summary>
  <p>An effect on an executor's behavior or outputs in a particular setting. Influence differs from the legitimacy, credibility, intended meaning, and usefulness of the influencing communication or condition.</p>
  </details>

- <details><summary>🗣️ Communication effect</summary>
  <p>An observable change associated with communication under specified conditions. Causation, usefulness, and mechanism require separate support.</p>
  </details>

- <details><summary>📡 Salience</summary>
  <p>The prominence of information to a recipient. It differs from relevance, credibility, authority, and demonstrated effect on an outcome.</p>
  </details>

- <details><summary>🛠️ Practical discretion</summary>
  <p>The usable opportunity to exercise authorized judgment, given actual evidence access, tools, resources, interfaces, and available transitions. Permission alone need not supply it.</p>
  </details>

- <details><summary>📦 Output contract</summary>
  <p>Requirements concerning a result's audience, destination, form, interface, or acceptance. It can express a genuine need or contain a premature substantive solution decision.</p>
  </details>

- <details><summary>🪜 Cognitive scaffolding</summary>
  <p>Additional directions intended to shape reasoning, exploration, planning, or self-review. Their contribution depends on the recipient, task, environment, and existing defaults.</p>
  </details>

- <details><summary>🤝 Shared meaning / shared understanding</summary>
  <p>Sufficient agreement about consequential meaning, scope, force, purpose, and conventions to support collaboration. It does not require identical internal representations, decompositions, or expertise.</p>
  </details>

- <details><summary>🗜️ Semantic compression</summary>
  <p>Communicating richer content through compact language or representation using shared interpretation. It does not imply a deterministic expansion into internal procedures.</p>
  </details>

- <details><summary>🧷 Candidate steering cue</summary>
  <p>A communicative element investigated for its behavioral effects. Calling it a candidate does not presume high impact, usefulness, or a known mechanism.</p>
  </details>

- <details><summary>📚 Grounding</summary>
  <p>Connection to relevant task reality, including source identity, currency, availability, provenance, scope, and consequential limitations. Factual credibility and instruction authority remain distinct.</p>
  </details>

- <details><summary>🗺️ Representation</summary>
  <p>A selective view of information or relationships that can enable useful operations while omitting other distinctions. Its existence does not establish completeness, truth, authority, or effective use.</p>
  </details>

- <details><summary>🔒 Representational commitment</summary>
  <p>Adopting or relying on a representation or answer form. A claim about its behavioral or internal effects needs a specified referent and appropriate evidence.</p>
  </details>

- <details><summary>⚓ Appropriate commitment</summary>
  <p>A design judgment that the timing, strength, scope, duration, and consequences of a commitment fit its purpose, evidence, authority, and dependencies. It does not invariably favor later or weaker commitment.</p>
  </details>

- <details><summary>⚠️ Material defect</summary>
  <p>A problem whose correction could change the objective, approach, commitment, risk, or truth of the result. Materiality does not imply discoverability before investigation.</p>
  </details>

- <details><summary>🔄 Reopenability</summary>
  <p>The ability to reconsider a commitment when material grounds emerge without permanently investigating every alternative or limiting revision to predefined triggers.</p>
  </details>

- <details><summary>🔧 Practical reopenability</summary>
  <p>The available ability to revise a decision and manage its dependencies and consequences, not merely permission to change its description or interpretation.</p>
  </details>

- <details><summary>🧪 Evidence horizon</summary>
  <p>The range of decisions or commitments currently available evidence can support. Later steps may depend on information not yet obtained.</p>
  </details>

- <details><summary>🗂️ Decision grounds</summary>
  <p>The rationale, evidence, assumptions, dependencies, accepted tradeoffs, and relevant authority needed to assess whether a decision still applies.</p>
  </details>

- <details><summary>🧠 Active context</summary>
  <p>Information available to an executor in its current interaction, distinct from everything retained in persistent storage, potentially obtainable through retrieval, or actually consulted.</p>
  </details>

- <details><summary>📖 Progressive disclosure</summary>
  <p>An arrangement providing orientation and access while bringing more detail into active use when relevant. Its value depends on discovering, retrieving, and correctly interpreting what matters.</p>
  </details>

- <details><summary>🏗️ Governing architecture</summary>
  <p>The arrangement through which purposes, information, authority, persistence, interfaces, and assurance shape work. It need not be a document, fixed taxonomy, or explicit set of modules.</p>
  </details>

- <details><summary>🧮 Derived support</summary>
  <p>Warranted support produced through inference, calculation, proof, or checking of supplied premises. It can be valuable without constituting an independent observation of those premises.</p>
  </details>

- <details><summary>🔗 Traceability</summary>
  <p>Inspectable relationships among specified sources, requirements, decisions, artifacts, and checks. Complete tracing of a declared inventory does not prove complete understanding of the task.</p>
  </details>

- <details><summary>📋 Scoped coverage / scoped completeness</summary>
  <p>Coverage or verification relative to a declared set, object, property, and assumptions. It does not automatically establish the adequacy of the inventory, correctness of interpretation, or completeness of understanding.</p>
  </details>

- <details><summary>🎭 Compliance theater</summary>
  <p>Checks, objections, explanations, or artifacts produced mainly to display diligence rather than improve the work, understanding, coordination, or justified reliance.</p>
  </details>

- <details><summary>🪶 No-additional-guidance baseline</summary>
  <p>A comparison without the extra layer under study while retaining the actual task, relevant context, permissions, environment, and existing governance. It is neither no governance nor a proven optimum.</p>
  </details>

- <details><summary>🧩 Model-specific compensation</summary>
  <p>Support intended to address an observed or suspected weakness of a particular model or configuration. Its cause, scope, benefit, cost, and continued necessity need appropriate evidence.</p>
  </details>

- <details><summary>📏 Ablation</summary>
  <p>A comparison removing or varying a component to investigate its contribution under identified conditions. Interpretation depends on controls, interactions, and measured outcomes.</p>
  </details>

- <details><summary>🔬 Assurance</summary>
  <p>Justified confidence that work can be relied upon for a particular purpose. Its strength depends on evidence, assumptions, and consequences rather than persuasion alone.</p>
  </details>

- <details><summary>🌉 Meaningful responsiveness / useful responsiveness</summary>
