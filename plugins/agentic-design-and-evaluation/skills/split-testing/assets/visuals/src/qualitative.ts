...
export function evidenceLineage(input: LineageInput): string {
  ...
  const plot = input.nodes.length ? svg(input.title, layout.height, relationships + boxes, layout.width) : '<p class="av-empty">No evidence nodes supplied.</p>';
  const edgeIdentity = (i: number) => identifier(edgeIds[i]) + (input.edges[i].id === undefined ? ' <span class="av-muted">(occurrence)</span>' : "");
  const inspector = `<details class="av-inspector" open><summary>Inspect evidence</summary><div class="av-object-list">${input.nodes.map((node, i) => objectDetail(`node-${i}`, `Node: ${node.label} · ${node.id}`, `<dl class="av-facts"><div><dt>Stable identity</dt><dd>${identifier(node.id)}</dd></div><div><dt>Supplied kind</dt><dd>${e(node.kind)}</dd></div></dl>${node.detail === undefined ? "" : `<p>${e(node.detail)}</p>`}${annotation(node)}${table("Supplied relationships for this object", ["Relationship ID / occurrence", "From ID", "Relation", "To ID", "Context"], input.edges.flatMap((edge, j) => edge.from === node.id || edge.to === node.id ? [[edgeIdentity(j), identifier(edge.from), e(edge.relation), identifier(edge.to), annotation(edge)]] : []))}`, i === 0)).join("")}${input.edges.map((edge, i) => objectDetail(`edge-${i}`, `Relationship: ${edge.relation} · ${edgeIds[i]}`, `<dl class="av-facts"><div><dt>Relationship ID / occurrence</dt><dd>${edgeIdentity(i)}</dd></div><div><dt>From</dt><dd>${identifier(edge.from)} — ${e(nodes.get(edge.from)!.label)}</dd></div><div><dt>To</dt><dd>${identifier(edge.to)} — ${e(nodes.get(edge.to)!.label)}</dd></div></dl><p class="av-relation-wording">${e(edge.relation)}</p>${annotation(edge)}`)).join("")}</div></details>`;
