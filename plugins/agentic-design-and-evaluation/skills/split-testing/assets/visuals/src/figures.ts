...
export function visualFigure(input: VisualFigureInput): string {
  ...
  return `<figure class="av-visual-figure" data-av-figure data-av-figure-title="${e(input.title)}"${input.id ? ` id="${e(documentId(input.id))}"` : ''}${input.adapter ? ` data-av-adapter="${e(input.adapter)}"` : ''}${input.source ? ` data-av-source="${e(JSON.stringify(input.source))}"` : ''}><figcaption class="av-figure-caption">${e(input.title)}${input.caption ? `<span>${e(input.caption)}</span>` : ''}</figcaption><div class="av-figure-body" data-av-figure-body>${input.body}</div></figure>`;
}
export interface MermaidDiagramInput { id?: string; title: string; source: string; caption?: string; config?: Record<string, unknown> }
export function mermaidDiagram(input: MermaidDiagramInput): string {
  ...
  const body = `<div class="av-mermaid" data-av-mermaid data-av-requires="mermaid" data-av-mermaid-source="${e(input.source)}"${input.config ? ` data-av-mermaid-config="${e(JSON.stringify(input.config))}"` : ''}><p class="av-note" data-av-mermaid-status role="status">Diagram source is available below.</p><div data-av-mermaid-output>${svg(input.title, 400, '', 900)}</div><details class="av-diagram-source"><summary>Diagram source</summary><pre>${e(input.source)}</pre></details></div>`;
