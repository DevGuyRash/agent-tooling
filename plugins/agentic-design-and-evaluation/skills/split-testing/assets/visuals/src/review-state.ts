...
export interface ReviewRecords { versions: readonly AnnotationVersion[]; bookmarks: readonly ReviewAnchor[] }
export type ReviewChange = { type:'annotation'; version: AnnotationVersion; observedIds: readonly string[] } | { type:'review-bookmark'; anchor: ReviewAnchor; enabled:boolean };
...
export function validateAnchor(value:unknown):ReviewAnchor{
  ...
  if(a.kind==='item'){if(!text(a.itemId))throw new Error('An item annotation needs its original item identity.');const result:ReviewAnchor={kind:'item',target,itemId:text(a.itemId),label:text(a.label),text:text(a.text)};if(a.values!==undefined){const data=object(a.values,Object.keys(a.values as object));if(!Object.values(data).every(item=>item===null||typeof item==='string'||typeof item==='number'&&Number.isFinite(item)))throw new Error('Review item values are invalid.');result.values={...data} as Record<string,string|number|null>;}return result;}
...
export function validateReview(value:unknown):ReviewRecords{
  const record=object(value,['versions','bookmarks']);if(!Array.isArray(record.versions)||!Array.isArray(record.bookmarks))throw new Error('Review records are invalid.');
  const ids=new Set<string>(),versions=array(record.versions).map(item=>{const v=object(item,['id','annotationId','anchor','text','at','draft','baseIds']);const id=text(v.id),annotationId=text(v.annotationId),at=readerTimestamp(v.at,'Annotation timestamp');if(!id||!annotationId||ids.has(id)||typeof v.draft!=='boolean')throw new Error('Review version identity or timestamp is invalid.');if(v.baseIds!==undefined&&!array(v.baseIds).every(id=>typeof id==='string'))throw new Error('Annotation draft bases are invalid.');ids.add(id);return{id,annotationId,anchor:validateAnchor(v.anchor),text:v.text===null?null:text(v.text),at,draft:v.draft,...(v.baseIds?{baseIds:array(v.baseIds).map(text)}:{})};});
  const bookmarks=array(record.bookmarks).map(validateAnchor);if(new Set(bookmarks.map(a=>JSON.stringify(a))).size!==bookmarks.length)throw new Error('Duplicate review bookmark.');return{versions,bookmarks};
}
export function applyReview(source:ReviewRecords,change:ReviewChange):ReviewRecords{
  const prior=validateReview(source);
  if(change.type==='review-bookmark'){const anchor=validateAnchor(change.anchor),key=JSON.stringify(anchor),bookmarks=prior.bookmarks.filter(item=>JSON.stringify(item)!==key);if(change.enabled)bookmarks.push(anchor);return{...prior,bookmarks};}
  ...
  if(existing){if(JSON.stringify(existing)!==JSON.stringify(version))throw new Error('Annotation version identity conflicts with saved content.');return prior;}
  const observed=new Set(array(change.observedIds).map(text));return{...prior,versions:[...prior.versions.filter(item=>item.annotationId!==version.annotationId||!observed.has(item.id)||(version.draft&&!item.draft)),version]};
}
