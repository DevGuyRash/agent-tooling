...
export function validateReview(value:unknown):ReviewRecords{
  ...
  const bookmarks=array(record.bookmarks).map(validateAnchor);if(new Set(bookmarks.map(a=>exactJson(a))).size!==bookmarks.length)throw new Error('Duplicate review bookmark.');return{versions,bookmarks};
}
export function applyReview(source:ReviewRecords,change:ReviewChange):ReviewRecords{
  ...
  if(existing){if(exactJson(existing)!==exactJson(version))throw new Error('Annotation version identity conflicts with saved content.');return prior;}
