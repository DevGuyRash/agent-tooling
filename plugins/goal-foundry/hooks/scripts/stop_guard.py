...
def main() -> int:
    ...
    if status.get("matched") is False:
        print(json.dumps({
            "decision": "block",
            "reason": "Goal Foundry audit gate: .goals/current.md hash no longer matches .goals/current.sha256. Do not claim success. Report 'contract mutated' with the expected/current hashes and stop for user review."
        }))
        return 0
    ...
    if CLAIM_WORDS.search(last):
        ...
        if missing:
            print(json.dumps({
                "decision": "block",
                "reason": "Goal Foundry final-report gate: before stopping, provide the required final report fields from .goals/current.md: files changed, commands run with raw pass/fail results or artifact paths, budget used, remaining risks, and follow-up candidates. Missing: " + ", ".join(missing)
            }))
            return 0
