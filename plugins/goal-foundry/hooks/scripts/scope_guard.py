...
def path_is_protected(path: str) -> bool:
    ...
    if any(p.startswith(prefix) for prefix in ALLOWED_GOALS_PREFIXES):
        return False
    return any(p == item or p.startswith(item + "/") for item in PROTECTED) or (p.startswith(".goals/") and not any(p.startswith(prefix) for prefix in ALLOWED_GOALS_PREFIXES))
...
def main() -> int:
    ...
    locked = current.exists() and lock.exists() and os.environ.get("GOAL_FOUNDRY_ALLOW_CONTRACT_WRITE") != "1"

    for pat in DANGEROUS_PATTERNS:
        if command and re.search(pat, command):
            deny(f"Goal Foundry blocked dangerous command matching {pat!r}.")
            return 0
    ...
    if tool in {"apply_patch", "Edit", "Write"} or tool == "apply_patch":
        paths = extract_patch_paths(command)
        protected_hits = [p for p in paths if path_is_protected(p)]
        if protected_hits:
            deny("Goal Foundry blocked modification to frozen goal artifacts: " + ", ".join(protected_hits) + ". Executor may only write .goals/evidence/ and .goals/reports/ during a run.")
            return 0

    if tool == "Bash" and command:
        hit = command_mentions_write_to_protected(command, PROTECTED + [".goals/"])
        if hit:
            deny(f"Goal Foundry blocked write-like command touching protected goal artifact: {hit}. Use $authoring-goals to revise and re-lock the contract before execution.")
            return 0
        if re.search(r"\b(git\s+checkout|git\s+reset|git\s+clean)\b", command) and ".goals" in command:
            deny("Goal Foundry blocked git command targeting .goals artifacts during an active run.")
            return 0
