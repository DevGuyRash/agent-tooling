...
from common import git_root_or_cwd, parse_sections
...
def scan_plugins(root: Path, home: Path, include_home: bool = True) -> List[dict]:
    ...
    candidates.extend([root / "plugins" / "goal-foundry", root])
    seen: Set[Path] = set()
    for d in candidates:
...
def scan_mcp(root: Path, home: Path, include_home: bool = True) -> List[dict]:
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            out.append({"name": "codex mcp list", "source": "codex-cli", "output": result.stdout.strip()[:4000]})
    except Exception:
        pass
    return out
