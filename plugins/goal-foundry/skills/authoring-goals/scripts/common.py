#!/usr/bin/env python3
"""Shared helpers for Goal Foundry scripts."""
...
def git_root_or_cwd(cwd: Optional[str] = None) -> Path:
    ...
    if os.environ.get("GOAL_FOUNDRY_NO_GIT") == "1":
        return start
...
def is_active_locked(cwd: Optional[str] = None) -> bool:
    _, current, lock = active_goal_paths(cwd)
    return current.exists() and lock.exists()
