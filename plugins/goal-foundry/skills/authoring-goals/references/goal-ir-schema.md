...
Goal Foundry internally normalizes raw intent, files, issues, PRDs, logs, and existing goals into this intermediate representation before rendering artifacts.
...
- `.goals/graph.json`: machine-readable relationships and statuses.
- `.goals/frontier.md`: discovery coverage and limits.
- Codex `/goal`: compact projection of `current.md`; it must not drop terminal-state, scope, budget, give-up, or evidence obligations.
