param(
    [string]$WorkbookPath = "$env:USERPROFILE\repos\example-workflows\excel\generic_workbook_fixture\workflow_fixture.xlsm",
    [int]$Rows = 1000,
    [int]$Iterations = 5,
    [double]$TimeoutSeconds = 30,
    [string]$SheetName = "DATA_RECORD_LINES",
    [string]$TableName = "tbl_record_lines"
)
...
function New-PasteData {
    ...
    for ($i = 0; $i -lt $RowCount; $i++) {
        ...
        $data[$i, 4] = $null
        $data[$i, 5] = $null
        $data[$i, 6] = "ASSET NUMBER $assetNumber PID NUMBER $pidValue BRAKE JOB"
    }
...
function Test-ManagedFormulasReady {
    ...
    foreach ($header in @("*Record ID", "Project Number", "Attribute 6")) {
...
function Get-ExpectedRecordCount {
    param($Data)
...
function Test-RecordSyncReady {
    ...
    if ([int]$RecordTable.ListRows.Count -lt $ExpectedRows) {
        return $false
    }

    $recordIdRange = $RecordTable.ListColumns.Item("*Record ID").DataBodyRange
    if ($recordIdRange -eq $null) {
        return $false
    }

    if (-not [bool]$recordIdRange.Cells.Item($ExpectedRows, 1).HasFormula) {
        return $false
    }
...
$expectedRecordAdds = Get-ExpectedRecordCount -Data $payload
...
try {
    ...
    for ($iteration = 1; $iteration -le $Iterations; $iteration++) {
        $tempPath = Join-Path $env:TEMP ("generic_workbook_fixture_bench_{0}_{1}.xlsm" -f $PID, $iteration)
        ...
        try {
            ...
            $expectedRecordRows = $initialRecordRows + $expectedRecordAdds

            $deferredReady = Wait-ForCondition -Stopwatch $stopwatch -TimeoutSeconds $TimeoutSeconds -Condition {
                Test-RecordSyncReady -Excel $excel -RecordTable $recordTable -ExpectedRows $expectedRecordRows
            }

            if (-not $deferredReady) {
                throw "Timed out waiting for deferred record sync to complete."
            }
