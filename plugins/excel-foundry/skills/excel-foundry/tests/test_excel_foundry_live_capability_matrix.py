...
def probe_excel_com(timeout: int = 180) -> tuple[bool, str]:
    ...
    try:
        proc = subprocess.run(
            [POWERSHELL_EXE or "powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", command],
            capture_output=True,
            text=True,
            timeout=timeout,
        )
...
@unittest.skipUnless(POWERSHELL_EXE, "PowerShell executable is required for Excel Foundry live matrix tests")
class ExcelFoundryLiveCapabilityMatrixTests(unittest.TestCase):
    ...
    def run_foundry(self, args: list[str], timeout: int = 180) -> dict[str, Any]:
        ...
        command = [POWERSHELL_EXE or "powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(LAUNCHER), *args]
