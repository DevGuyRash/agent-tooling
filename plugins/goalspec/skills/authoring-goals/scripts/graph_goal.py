...
def load(path: Path) -> dict:
    if path.exists():
        try:
            ...
            pass
    return {"schema": "goalspec.graph.v1", "created": date.today().isoformat(), "nodes": {}, "edges": []}


def contract_metadata(contract: Path) -> tuple[str, dict]:
    text = contract.read_text(encoding="utf-8")
    m = re.search(r"^#\s+Goal Contract:\s*(.+?)\s*$", text, re.M)
    raw_title = m.group(1).strip() if m else contract.stem
    gid_part, title = split_goal_id_title(raw_title)
    gid = gid_part or contract.stem
