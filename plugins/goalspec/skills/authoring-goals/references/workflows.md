...
6. **Render and hand off.** Human-stepped: compile at most one ready child into `.goals/current.md`, validate, lock, render with `--pointer`. Autonomous: `render_goal.py --campaign <manifest> --pointer`. Either render freezes the chain runtime into `.goals/bin/` and writes the initial `.goals/focus.md` (the executor's first read: current goal, outcome tree, cursor commands). End your final message with the launch line verbatim — that line is the entire handoff; do not create or edit a host thread goal during authoring. If no child is ready, stop at the campaign and report the decision blocking the most promising child.
