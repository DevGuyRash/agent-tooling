...
if [ -z "${compose_skip_flag}" ] && [ -n "${AGENT_SKILLS_SKIP_PCA_BUILD:-}" ]; then
  ...
  log "maintenance" "warning: AGENT_SKILLS_SKIP_PCA_BUILD is deprecated; use AGENT_SKILLS_SKIP_DOCKER_ARCHITECT_COMPOSE_BUILD"
fi
...
if [ -z "${image_skip_flag}" ] && [ -n "${AGENT_SKILLS_SKIP_PIASCS_BUILD:-}" ]; then
  ...
  log "maintenance" "warning: AGENT_SKILLS_SKIP_PIASCS_BUILD is deprecated; use AGENT_SKILLS_SKIP_DOCKER_ARCHITECT_IMAGE_BUILD"
fi
