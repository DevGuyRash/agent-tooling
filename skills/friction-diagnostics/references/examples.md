...
This file does not exist before the first recorded event. A manifest-only `init-log.*` session leaves no `INDEX.md`, `events.jsonl`, `incidents.json`, descriptor, or markdown log behind.
