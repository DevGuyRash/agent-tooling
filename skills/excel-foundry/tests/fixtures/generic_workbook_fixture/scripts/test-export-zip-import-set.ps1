...
try {
    $tempPath = Join-Path $env:TEMP ("generic_workflow_export_import_set_{0}.xlsm" -f $PID)
    Copy-Item -LiteralPath $WorkbookPath -Destination $tempPath -Force

    $singleZipPath = Join-Path $env:TEMP ("generic_workflow_export_single_{0}.zip" -f $PID)
    $multiZipPath = Join-Path $env:TEMP ("generic_workflow_export_multi_{0}.zip" -f $PID)
    ...
    $importSetRange = $recordTable.ListColumns.Item("Import Set").DataBodyRange
    ...
    if ([string]::IsNullOrWhiteSpace($expectedSingle)) {
        throw "Expected a nonblank Import Set in the first record row."
    }
