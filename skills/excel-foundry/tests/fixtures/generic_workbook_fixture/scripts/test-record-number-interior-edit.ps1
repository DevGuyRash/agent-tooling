...
try {
    $tempPath = Join-Path $env:TEMP ("generic_workflow_record_number_interior_edit_{0}.xlsm" -f $PID)
    ...
    $initialLineRows = [int]$linesTable.ListRows.Count
    ...
    $firstDescriptionCell = $linesTable.ListColumns.Item("Description").DataBodyRange.Cells.Item($initialLineRows + 1, 1)
    $firstDescriptionCell.Value2 = "ASSET NUMBER 2004500999 LABOR ONLY"

    Wait-ForCondition -Stopwatch $stopwatch -TimeoutSeconds $TimeoutSeconds -FailureMessage "Timed out waiting for live record recalculation after a line description edit." -Condition {
        try {
            Assert-RecordNumbers -RecordTable $recordsTable -ExpectedNumbers $expectedAfterInteriorEdit
            return $true
        }
        catch {
            return $false
    ...
    Write-Output "Interior edit record-number recalculation regression check passed."
}
finally {
    if ($workbook -ne $null) {
