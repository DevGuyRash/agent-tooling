...
'     - tbl_invoice_lines becomes grow-only during editing (no auto-shrink)
'     - managed formula columns are hard-coded (no .HasFormula guessing)
'     - large pastes queue invoice sync and formatting work for deferred flush
...
Private Const COL_INV_NUMBER_FORMULA As String = "*Invoice Number"
...
Private mDeferredInvoiceSyncPending As Boolean
Private mFlushDeferredAfterExitPending As Boolean
Private mSavedLinesEnableCalculation As Variant
Private mSavedInvoicesEnableCalculation As Variant
Private mHaveSavedLinesEnableCalculation As Boolean
Private mHaveSavedInvoicesEnableCalculation As Boolean
...
Public Sub AP_SyncInvoicesTable(Optional ByVal Silent As Boolean = True, _
    ...
    If InvoiceKeysMatchTable(lcInvoiceKey, invoiceKeys) Then GoTo FinalizeInvoices
...
FinalizeInvoices:
    RecalculateInvoiceTable loInv
    ApplyInvoiceNumberBordersFromPreviousRow loInv, appendedStartRow, appendedEndRow
...
Public Sub AP_SyncAll_WithTarget(Optional ByVal Silent As Boolean = True, _
    ...
    Dim handledInvoiceSync As Boolean
    ...
    If runInvoiceSync Then
        handledInvoiceSync = TrySyncInvoicesTableTargeted(Silent, changedBottomRow, changedTopRow, changedFirstCol, changedLastCol, protectChangedRows)

        If Not handledInvoiceSync Then
...
Private Function TrySyncInvoicesTableTargeted(ByVal Silent As Boolean, _
    ...
    Set wsInv = wb.Worksheets(SH_INVOICES)
    ...
    ResolveAffectedInvoiceRowBounds loLines, usedLineRows, changedTopRow, changedBottomRow, protectChangedRows, affectedStartRow, affectedEndRow

    If Not protectChangedRows Then
        affectedEndRow = desiredRows
    End If

    ' Invoice-number sequencing depends on prior invoice rows, so any affected
    ' invoice row can change the numbering for the rest of the table.
    If affectedStartRow > 0 Then
        affectedEndRow = desiredRows
    End If

    If InvoiceKeysMatchTable(lcInvoiceKey, invoiceKeys) Then
        RecalculateInvoiceTableSlice loInv, affectedStartRow, affectedEndRow
        TrySyncInvoicesTableTargeted = True
        Exit Function
    End If
...
FallbackFullSync:
    TrySyncInvoicesTableTargeted = False
End Function
...
Public Sub AP_QueueSyncAll_WithTarget(Optional ByVal Silent As Boolean = True, _
    ...
    Dim invoiceAffecting As Boolean
    ...
    invoiceAffecting = ChangeMayAffectInvoices(changedBottomRow, changedTopRow, changedFirstCol, changedLastCol, protectChangedRows)
    ...
    If Not mSyncPending Then
        ...
        mPendingRunInvoiceSync = invoiceAffecting
...
EH:
    ...
    AP_SyncAll_WithTarget Silent, changedBottomRow, changedTopRow, changedFirstCol, changedLastCol, protectChangedRows, invoiceAffecting
End Sub
...
Public Sub AP_RunQueuedSync()
    ...
    If Not mSyncPending Then
        ...
        If shouldFlushDeferredAfterRun Then
            If mDeferredLineFormulasPending Or mDeferredLineCfPending Or mDeferredInvoiceSyncPending Then
                mSyncInFlight = True
...
CleanExit:
    ...
    If mSyncPending Then
        Application.StatusBar = STATUS_SYNC_PENDING
        ScheduleQueuedSyncRun
    ElseIf mDeferredLineFormulasPending Or mDeferredLineCfPending Or mDeferredInvoiceSyncPending Then
        Application.StatusBar = STATUS_SYNC_FAST_EDIT & " (deferred refresh)"
    Else
        Application.StatusBar = False
    End If
...
Public Sub AP_HandleLinesSheetDeactivate(Optional ByVal Silent As Boolean = True)
    ...
    If Not mSyncPending And _
       ...
       Not mDeferredInvoiceSyncPending Then
        mFlushDeferredAfterExitPending = False
        Application.StatusBar = False
        Exit Sub
    End If
...
Public Sub AP_SetPerformanceEditMode(ByVal enabled As Boolean)
    ...
    If enabled Then
        ...
        If Not mHaveSavedInvoicesEnableCalculation Then
            On Error Resume Next
            mSavedInvoicesEnableCalculation = wsInv.EnableCalculation
            mHaveSavedInvoicesEnableCalculation = (Err.Number = 0)
            Err.Clear
            On Error GoTo SafeExit
        End If
    ...
    Else
        ...
        If mHaveSavedInvoicesEnableCalculation Then
            On Error Resume Next
            wsInv.EnableCalculation = CBool(mSavedInvoicesEnableCalculation)
            Err.Clear
            On Error GoTo SafeExit
        End If
...
Private Sub FlushDeferredLineWorkCore(ByVal Silent As Boolean, _
    ...
    If Not mDeferredLineFormulasPending And Not mDeferredLineCfPending And Not mDeferredInvoiceSyncPending Then Exit Sub
    ...
    If mDeferredInvoiceSyncPending Then
        AP_SyncInvoicesTable Silent, True, False
    End If

    mDeferredLineFormulasPending = False
    mDeferredLineCfPending = False
    mDeferredInvoiceSyncPending = False
...
Private Sub ResolveAffectedInvoiceRowBounds(ByVal loLines As ListObject, _
                                            ByVal usedRows As Long, _
                                            ByVal changedTopRow As Long, _
                                            ByVal changedBottomRow As Long, _
    ...
    Dim invoiceIx As Long
    Dim currentInvoiceStart As Long
    currentInvoiceStart = 1

    Dim rowIndex As Long
    For rowIndex = 1 To usedRows
        If IsLineStartValue(lineNumberValues(rowIndex, 1)) Then
            invoiceIx = invoiceIx + 1

            If rowIndex <= localStart Then
                currentInvoiceStart = invoiceIx
            End If
        End If

        If rowIndex = localStart Then
            startIx = MaxLong(1, currentInvoiceStart)
        End If

        If rowIndex >= localEnd Then
            endIx = MaxLong(startIx, invoiceIx)
            Exit Sub
        End If
    Next rowIndex

    If protectChangedRows And changedBottomRow > (firstDataRow + usedRows - 1) Then
        endIx = MaxLong(endIx, invoiceIx + 1)
    Else
        endIx = MaxLong(startIx, invoiceIx)
    End If
End Sub

Private Sub ApplyInvoiceTableFormulas(ByVal loInv As ListObject)
...
Private Sub ApplyInvoiceTableFormulasSlice(ByVal loInv As ListObject, ByVal startIx As Long, ByVal endIx As Long)
    If loInv Is Nothing Then Exit Sub
    If loInv.DataBodyRange Is Nothing Then Exit Sub
...
Private Sub RecalculateInvoiceTable(ByVal loInv As ListObject)
    If loInv Is Nothing Then Exit Sub
    If loInv.DataBodyRange Is Nothing Then Exit Sub

    ApplyInvoiceTableFormulas loInv
    CalculateManagedInvoiceColumns loInv, COL_INV_NUMBER_FORMULA
    CalculateInvoiceManagedColumn loInv, COL_INV_NUMBER_FORMULA
    loInv.DataBodyRange.Calculate
End Sub

Private Sub RecalculateInvoiceTableSlice(ByVal loInv As ListObject, ByVal startIx As Long, ByVal endIx As Long)
    If loInv Is Nothing Then Exit Sub
    If loInv.DataBodyRange Is Nothing Then Exit Sub
    ...
    ApplyInvoiceTableFormulasSlice loInv, startIx, endIx
    CalculateManagedInvoiceColumnsSlice loInv, startIx, endIx, COL_INV_NUMBER_FORMULA
    CalculateInvoiceManagedColumnSlice loInv, COL_INV_NUMBER_FORMULA, startIx, endIx
    loInv.DataBodyRange.Rows(startIx & ":" & endIx).Calculate
End Sub

Private Sub CalculateManagedInvoiceColumnsSlice(ByVal loInv As ListObject, _
                                                ByVal startIx As Long, _
...
Private Sub CalculateInvoiceManagedColumnSlice(ByVal loInv As ListObject, _
...
Private Sub CalculateManagedInvoiceColumns(ByVal loInv As ListObject, Optional ByVal skipHeaderName As String = "")
    If loInv Is Nothing Then Exit Sub
    If loInv.DataBodyRange Is Nothing Then Exit Sub
...
Private Sub CalculateInvoiceManagedColumn(ByVal loInv As ListObject, ByVal headerName As String)
    If loInv Is Nothing Then Exit Sub
    If loInv.DataBodyRange Is Nothing Then Exit Sub
...
Private Sub DisableManagedSheetsCalculationForFastEdit()
    ...
    If mHaveSavedInvoicesEnableCalculation Then
        If CBool(mSavedInvoicesEnableCalculation) Then wsInv.EnableCalculation = False
    Else
...
Private Function IsManagedFormulaColumn(ByVal tableName As String, ByVal headerName As String) As Boolean
    ...
    Select Case t
        ...
        Case LCase$(TBL_INVOICES)
            Select Case h
                Case LCase$("*Invoice ID"), _
                     LCase$("*Business Unit"), _
                     LCase$("*Source"), _
                     LCase$("*Invoice Number"), _
                     LCase$("*Invoice Amount"), _
                     LCase$("*Invoice Date"), _
                     LCase$("*Supplier Site"), _
                     LCase$("Invoice Currency"), _
                     LCase$("Payment Currency"), _
                     LCase$("Description"), _
                     LCase$("Import Set"), _
                     LCase$("*Invoice Type"), _
                     LCase$("Legal Entity"), _
                     LCase$("*Payment Terms"), _
                     LCase$("Invoice Received Date"), _
                     LCase$("Accounting Date"), _
...
Private Function ChangeMayAffectInvoices(ByVal changedBottomRow As Long, _
                                         ByVal changedTopRow As Long, _
                                         ByVal changedFirstCol As Long, _
                                         ByVal changedLastCol As Long, _
                                         ByVal protectChangedRows As Boolean) As Boolean
    ChangeMayAffectInvoices = (changedTopRow > 0 And changedBottomRow >= changedTopRow)
End Function
...
Private Sub ApplyInvoiceNumberBordersFromPreviousRow(ByVal loInv As ListObject, _
                                                     ByVal startRow As Long, _
                                                     ByVal endRow As Long)
    If loInv Is Nothing Then Exit Sub
    ...
    Dim invoiceNumberColumn As ListColumn
    Set invoiceNumberColumn = GetListColumn(loInv, COL_INV_NUMBER_FORMULA)
    If invoiceNumberColumn Is Nothing Then Exit Sub
    If invoiceNumberColumn.DataBodyRange Is Nothing Then Exit Sub

    If endRow > invoiceNumberColumn.DataBodyRange.Rows.Count Then
        endRow = invoiceNumberColumn.DataBodyRange.Rows.Count
    End If

    Dim rowIndex As Long
    For rowIndex = startRow To endRow
        CopyCellBorderStyles invoiceNumberColumn.DataBodyRange.Cells(rowIndex - 1, 1), _
                             invoiceNumberColumn.DataBodyRange.Cells(rowIndex, 1)
    Next rowIndex
End Sub
