...
try {
    $tempPath = Join-Path $env:TEMP ("tr_upload_invoice_number_interior_edit_{0}.xlsm" -f $PID)
    ...
    $firstDescriptionCell.Value2 = "STOCK NUMBER 2004500999 LABOR ONLY"

    Wait-ForCondition -Stopwatch $stopwatch -TimeoutSeconds $TimeoutSeconds -FailureMessage "Timed out waiting for live invoice recalculation after a line description edit." -Condition {
        try {
            Assert-InvoiceNumbers -InvoiceTable $invoicesTable -ExpectedNumbers $expectedAfterInteriorEdit
            return $true
        }
        catch {
            return $false
    ...
    Write-Output "Interior edit invoice-number recalculation regression check passed."
}
finally {
    if ($workbook -ne $null) {
