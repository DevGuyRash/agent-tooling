...
1. **Anchor verification**: Spawn an explorer subagent to confirm the finding's `file:line` exists and contains the claimed code. IF wrong or fabricated → DECLINE.
...
|---|---|---|
| APPLIED | Finding passed evidence challenge; will implement | `mpcr applicator note --reviewer-id <ID8> --session-id <ID8> --note-type applied --content "finding: <title>, verified: <how>, action: <what>"` |
| DECLINED | Finding failed evidence challenge | `mpcr applicator note --reviewer-id <ID8> --session-id <ID8> --note-type declined --content "finding: <title>, challenge: <which check failed>, reason: <evidence>"` |
| DEFERRED | Verified but out of scope | `mpcr applicator note --reviewer-id <ID8> --session-id <ID8> --note-type deferred --content "finding: <title>, tracking: <ref>"` |
| ALREADY_ADDRESSED | Already fixed elsewhere | `mpcr applicator note --reviewer-id <ID8> --session-id <ID8> --note-type already_addressed --content "finding: <title>, ref: <where>"` |
...
`mpcr applicator set-status --reviewer-id <ID8> --session-id <ID8> --initiator-status REVIEWED`
...
4. You SHALL respect a concurrency cap of 8 parallel applicator subagents.
5. Launch applicator-worker subagents in parallel.
6. Collect Application Reports from each worker.
7. Optionally launch applicator-verifier subagents to spot-check fixes.
...
Artifact cleanup:
- AFTER finalization, you SHALL delete `.local/tmp/` if it exists: `find .local/tmp -delete 2>/dev/null || true`
- You SHALL NOT leave dispatch prompts, report templates, or scratch artifacts in the worktree.
...
mpcr applicator note --reviewer-id <ID8> --session-id <ID8> --note-type <TYPE> --content "..."
