...
You SHALL use `mpcr` for all operations and interactions regarding the `_session.json` file. The CLI is located at `<skills-file-root>/scripts/mpcr` (it auto-compiles on first run; requires `cargo`). In the examples below, `mpcr` refers to that path. Run `mpcr --help` for full command reference.
...
> Note: `--print-env` and `--emit-env sh` are mutually exclusive; choose exactly one output mode.

> **Session directory determinism**
>
> `mpcr reviewer register ...` defaults to `today in UTC` when computing the default session directory.
> IF you are resuming a review started on a different date (or from a non-default session dir) THEN you SHALL pass `--date <YYYY-MM-DD>` or `--session-dir <DIR>` explicitly on `register`.
...
- `MPCR_PARENT_ID` (optional; only printed when `--parent-id` is set)
...
```sh
mpcr reviewer register --target-ref '<NEW_REF>' --reviewer-id <REVIEWER_ID8> --print-env
```

Where `<REVIEWER_ID8>` is the `MPCR_REVIEWER_ID` value you captured earlier.

4) Use the printed values explicitly in later commands:

```sh
mpcr reviewer update --session-dir <DIR> --reviewer-id <REVIEWER_ID8> --session-id <SESSION_ID8> --status IN_PROGRESS --phase INGESTION
```

Where `<DIR>` is `MPCR_SESSION_DIR` and `<SESSION_ID8>` is `MPCR_SESSION_ID` from `--print-env`.

### 0.6 Parent/child reviewer lineage (Child Reviewers)

IF you are an orchestrator delegating a Child Reviewer THEN you SHALL:

- provide your own `reviewer_id` to the child as `parent_id`
- ensure the child joins the exact intended session by passing `--session-dir` and `--session-id` explicitly
- ensure each Child Reviewer uses a distinct `reviewer_id` within the session:
...
Preferred (deterministic; reduces manual ID handling): the orchestrator SHALL use `mpcr reviewer spawn-children` after registering so `mpcr` creates child entries with `parent_id` recorded and returns the assigned child `reviewer_id` values.

```sh
mpcr reviewer spawn-children --target-ref '<REF>' --session-dir <DIR> --session-id <SESSION_ID8> --parent-id <PARENT_REVIEWER_ID8> --count <N> --json
```

IF you are a Child Reviewer spawned by an orchestrator THEN you SHALL:

- set `parent_id` to the orchestrator’s `reviewer_id` when registering so lineage is recorded in `_session.json`
- use your assigned `reviewer_id` (either provided by the orchestrator OR captured from your own `mpcr reviewer register --print-env` output) for all subsequent `mpcr reviewer ...` commands

To ensure the child joins the *exact* intended session, you SHALL pass `--session-dir` and `--session-id` explicitly (capture both from the orchestrator’s `--print-env` output).
...
mpcr reviewer update --session-dir <DIR> --reviewer-id <REVIEWER_ID8> --session-id <SESSION_ID8> --phase DOMAIN_COVERAGE
# ... perform domain coverage work ...
mpcr reviewer update --session-dir <DIR> --reviewer-id <REVIEWER_ID8> --session-id <SESSION_ID8> --phase THEOREM_GENERATION
# ... perform theorem generation work ...
mpcr reviewer update --session-dir <DIR> --reviewer-id <REVIEWER_ID8> --session-id <SESSION_ID8> --phase ADVERSARIAL_PROOFS
# ... perform adversarial proofs work ...
mpcr reviewer update --session-dir <DIR> --reviewer-id <REVIEWER_ID8> --session-id <SESSION_ID8> --phase SYNTHESIS
# ... perform synthesis work ...
mpcr reviewer update --session-dir <DIR> --reviewer-id <REVIEWER_ID8> --session-id <SESSION_ID8> --phase REPORT_WRITING
```

IF you lose the context values THEN re-run `mpcr reviewer register --target-ref '<REF>' --reviewer-id <REVIEWER_ID8> --print-env` to reprint them.
...
```sh
mpcr reviewer note --session-dir <DIR> --reviewer-id <REVIEWER_ID8> --session-id <SESSION_ID8> --note-type blocker_preview --content "Merge conditions: run required checks and add evidence to the report. Anchors: crates/foo/src/bar.rs:123 (fn handle_request), crates/foo/src/baz.rs:77 (impl Service::call)."
```

### 3) Finalize without leaving scratch files

You SHALL finalize via `mpcr reviewer finalize` and you SHALL record accurate severity counts (they SHALL match the report).
...
```sh
mpcr reviewer finalize --session-dir <DIR> --reviewer-id <REVIEWER_ID8> --session-id <SESSION_ID8> --verdict REQUEST_CHANGES --major 2 <<'EOF'
...
# ... read code, analyze ...
mpcr reviewer update --session-dir <DIR> --reviewer-id <REVIEWER_ID8> --session-id <SESSION_ID8> --phase REPORT_WRITING
mpcr reviewer finalize --session-dir <DIR> --reviewer-id <REVIEWER_ID8> --session-id <SESSION_ID8> --verdict APPROVE --blocker 0 --major 0 --minor 0 --nit 1 <<'EOF'
## Adversarial Code Review: <Ref>
... full report content ...
EOF
```
...
- Parent-child chains when spawned by another reviewer (recorded via `parent_id`)
...
#   mpcr reviewer finalize --session-dir <DIR> --reviewer-id <REVIEWER_ID8> --session-id <SESSION_ID8> --report-file "$SCRATCH_REPORT" --verdict APPROVE

rm -r ".local/scratch/code_reviews/<reviewer_id>/<session_id>"
# Optional: cleanup all scratch for this reviewer id:
#   rm -r ".local/scratch/code_reviews/<reviewer_id>"
```
