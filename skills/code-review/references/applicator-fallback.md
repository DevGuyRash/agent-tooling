...
- MINOR/NIT findings SHALL NOT reopen the workflow by themselves. WHEN they are verified but not required for this patch, DEFER them or batch them only when a full-cycle terminal minor cleanup explicitly requests it.
...
4. Launch applicator-worker subagents in parallel (with dynamic worker budget (baseline 4; probe 6 and 8 cautiously when cycle health is stable)).
...
9. In standalone applicator mode, you SHALL apply only the assigned findings. You SHALL NOT self-expand into adjacent cleanup unless the user explicitly requested broader polish.
...
7. Only BLOCKER/MAJOR regressions or behavior-facing staleness MAY reopen the workflow. Record MINOR/NIT side effects as follow-up or deferred notes instead of spawning another recursive cleanup pass.
8. Return your Application Report.
...
- Applied required-now findings: N
- Applied terminal MINOR/NIT cleanup findings: N
...
5. Convergence reopen bar: only BLOCKER/MAJOR findings and staleness mismatches that affect shipped behavior or operator guidance reopen the workflow.
6. IF the reopen bar is non-zero, recurse another cycle on those required-now findings only.
7. IF the reopen bar is zero and only verified MINOR/NIT findings remain, batch them into one terminal cleanup pass, then allow exactly one final delta-only re-review.
8. IF that final check reports only MINOR/NIT (or no findings), stop and carry any remainder as follow-up or residual polish.
