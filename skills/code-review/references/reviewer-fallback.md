...
## Verdict (pick ONE)
- **SHIP**: all findings are NIT-only or no findings. No changes required before merge.
- **SHIP_WITH_FIXES**: findings exist (MINOR+) but are all fixable in this PR cycle. List the required fixes.
- **DO_NOT_SHIP**: BLOCKER or MAJOR findings that require design rethinking, or cumulative MINOR risk is too high. List blocking items.

## Evaluation matrix

| Axis | PASS | CONDITIONAL | FAIL |
|------|------|-------------|------|
| Correctness | No BLOCKER findings | BLOCKERs have clear, bounded fixes | Unfixable correctness flaw |
| Safety | No security/data-integrity BLOCKERs | Security BLOCKERs with mitigations proposed | Unmitigated security flaw |
| Complexity budget | No MAJOR overengineering or O(n^2+) on hot paths | MAJORs with agreed refactor plan | Architectural overengineering blocking comprehension |
| Test coverage | New code has tests; existing tests pass | Missing tests for edge cases (MINOR) | No tests for new behavior |
| Acceptance criteria | All criteria met | Partial; gaps are MINOR and tracked | Critical criteria unmet |
