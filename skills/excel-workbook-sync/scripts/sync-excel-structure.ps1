...
function Ensure-TableDefinition {
    ...
    try {
        ...
        $listObject = Find-ListObjectByName -Worksheet $worksheet -TableName ([string]$TableSpec.name)

        if ($null -eq $listObject) {
            $start = Get-RangeFromAddress -Worksheet $worksheet -Address ([string]$TableSpec.topLeft)
            $targetRange = Set-RangeValues -Range $start -Values $allRows
            $listObject = $worksheet.ListObjects.Add(1, $targetRange, $null, 1)
            $listObject.Name = [string]$TableSpec.name
        }
        else {
            $start = $worksheet.Range(([string]$TableSpec.topLeft))
            ...
            for ($col = 1; $col -le $headerValues.Count; $col++) {
                $listObject.ListColumns.Item($col).Name = [string]$headerValues[$col - 1]
            }

            $dataRows = @($allRows | Select-Object -Skip 1)
            if ($dataRows.Count -gt 0) {
                [void](Set-RangeValues -Range $start.Offset(1, 0) -Values $dataRows)
            }
        }
    }
    catch {
