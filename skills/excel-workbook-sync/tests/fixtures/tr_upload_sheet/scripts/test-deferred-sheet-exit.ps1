...
try {
    $tempPath = Join-Path $env:TEMP ("tr_upload_deferred_sheet_exit_{0}.xlsm" -f $PID)
    ...
    Wait-ForCondition -Stopwatch $stopwatch -TimeoutSeconds $TimeoutSeconds -FailureMessage "Timed out waiting for live line-sheet formulas on the active sheet." -Condition {
        $lineInvoiceIdRange = $linesTable.ListColumns.Item("*Invoice ID").DataBodyRange
        return [bool]$lineInvoiceIdRange.Cells.Item($pastedLineIndex, 1).HasFormula
    }

    if ([string]$excel.ActiveSheet.Name -ne "AP_INVOICE_LINES_INTERFACE") {
        throw "Expected to remain on AP_INVOICE_LINES_INTERFACE during live sync."
    }

    Wait-ForCondition -Stopwatch $stopwatch -TimeoutSeconds $TimeoutSeconds -FailureMessage "Timed out waiting for live invoice rows while staying on the lines sheet." -Condition {
        return ((Get-TableRowCount -Table $invoicesTable) -ge 8)
    }

    Wait-ForCondition -Stopwatch $stopwatch -TimeoutSeconds $TimeoutSeconds -FailureMessage "Timed out waiting for invoice ID formulas while staying on the lines sheet." -Condition {
        $invoiceIdRange = $invoicesTable.ListColumns.Item("*Invoice ID").DataBodyRange
        return [bool]$invoiceIdRange.Cells.Item(8, 1).HasFormula
    }

    if ([string]$excel.ActiveSheet.Name -ne "AP_INVOICE_LINES_INTERFACE") {
        throw "Invoice sync unexpectedly forced a sheet switch away from AP_INVOICE_LINES_INTERFACE."
    }

    Write-Output "Live lines-sheet invoice sync regression check passed."
}
finally {
    if ($workbook -ne $null) {
