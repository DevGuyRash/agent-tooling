...
fn readiness_timeout_for_services(
    ...
    services
        ...
        .map_or(base_timeout, |budget| budget)
}
...
fn service_health_budget(service: &ServiceReadinessRecord) -> Option<Duration> {
    ...
    let start_period = match service.healthcheck_start_period {
        Some(value) => value,
        None => Duration::default(),
    };
    let interval = match service.healthcheck_interval {
        Some(value) => value,
        None => Duration::default(),
    };
    let timeout = match service.healthcheck_timeout {
        Some(value) => value,
        None => Duration::default(),
    };
    let retries = service
