...
pub(crate) fn discover_composite_anchors(
    ...
    for candidate in raw {
        ...
        let (name, yaml_key) = composite_name_and_key(
            candidate.keys.first().map_or("fragment", String::as_str),
...
pub fn suggest_anchors(
    ...
    let threshold = match opts.min_usage_override {
        Some(value) => value,
        None => adaptive_threshold(service_count),
    };
    ...
    for (index, item) in prepared.into_iter().enumerate() {
        let (mut proposed_name, key_root) = match item.kind {
            SuggestionKind::Composite => {
                let name = match item.name_hint {
                    Some(name) => name,
                    None => "composite_fragment".to_string(),
                };
                let key = match item.yaml_key_hint {
                    Some(key) => key,
                    None => "x-composite-fragment".to_string(),
                };
                (name, key)
            }
...
fn key_count_for_fragment(fragment: &YamlValue) -> usize {
    fragment.as_mapping().map_or(1, Mapping::len).max(1)
}

fn top_level_fragment_keys(fragment: &YamlValue) -> Vec<String> {
    let mut keys = if let Some(map) = fragment.as_mapping() {
        map.keys()
            .filter_map(YamlValue::as_str)
            .map(ToOwned::to_owned)
            .collect::<Vec<String>>()
