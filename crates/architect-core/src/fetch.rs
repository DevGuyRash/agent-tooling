...
fn fetch_image_profile_with_client(
    ...
    let mut researched_config =
        match knowledge::researched_config_for_image(&parsed.normalized, &runtime.signatures)? {
            Some(config) => config,
            None => ResearchedConfig::default(),
        };
    if allow_scrape_fallback {
...
fn fetch_bearer_token(
...

    {
        ...
        let scope = match challenge.scope.as_deref().map(ToOwned::to_owned) {
            Some(scope) => scope,
            None => format!("repository:{repository}:pull"),
        };
...
fn extract_runtime_profile_from_config_payload(value: &Value) -> RuntimeProfile {
    ...
    let config = value.get("config").map_or(value, |config| config);
    profile.user = config
        .get("User")
        .and_then(Value::as_str)
        .map(ToOwned::to_owned)
