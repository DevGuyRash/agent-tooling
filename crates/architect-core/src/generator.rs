...
fn key_count_for_body(body: &YamlValue) -> usize {
    body.as_mapping().map_or(1, Mapping::len).max(1)
}
...
fn select_default_anchor_names(
    ...
    for definition in defaults {
        let usage = usage_counts.get(&definition.name).map_or(0, |count| *count);
...
fn top_level_keys(body: &YamlValue) -> Vec<String> {
    let mut keys = if let Some(map) = body.as_mapping() {
        map.keys()
            .filter_map(YamlValue::as_str)
            .map(ToOwned::to_owned)
            .collect::<Vec<String>>()
...
fn compare_anchor_names(
    ...
    let left_rank = left_def.map_or(2, anchor_source_rank);
    let right_rank = right_def.map_or(2, anchor_source_rank);
    ...
    let left_yaml = left_def
        ...
        .map_or(left, |value| value);
    let right_yaml = right_def
        .map(|definition| definition.yaml_key.as_str())
        .map_or(right, |value| value);
