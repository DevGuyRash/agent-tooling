...
pub fn ensure_volume_permissions(
    ...
    let init_mounts: Vec<String> = mounts
        ...
        .map(|(index, mount)| {
            format!(
                "{}:/mnt/perm-{index}",
                mount.source.as_deref().map_or("", |source| source)
            )
        })
        .collect();
...
pub fn ensure_writable_paths(
    ...
    for spec in BASELINE_TMPFS_SPECS {
        let target = spec.split(':').next().map_or(spec, |value| value);
    ...
    for target in required_persistent_targets {
        if BASELINE_TMPFS_SPECS
            .iter()
            .map(|spec| spec.split(':').next().map_or(*spec, |value| value))
...
pub fn synthesize_healthcheck(
    ...
    let distroless = profile.is_some_and(|image| image.runtime.signatures.distroless);
    let template = [
    ...
    vec![HeuristicPatch {
        ...
        value: json!({
          ...
          "interval": match healthcheck.interval {
              Some(value) => value,
              None => "10s".to_string(),
          },
          "timeout": match healthcheck.timeout {
              Some(value) => value,
              None => "5s".to_string(),
          },
          "start_period": match healthcheck.start_period {
              Some(value) => value,
              None => "10s".to_string(),
          },
          "retries": healthcheck.retries.map_or(5, |value| value)
        }),
...
fn collect_service_tmpfs_targets(service: &Mapping) -> std::collections::BTreeSet<String> {
    ...
    for item in items {
        ...
        let target = spec.split(':').next().map_or(spec, |value| value).trim();
        if !target.is_empty() {
...
fn healthcheck_from_tools(profile: &ImageProfile) -> Option<ComposeHealthcheckTemplate> {
    let tool_available = |tool: &str| {
        if let Some(item) = profile.runtime.tool_details.get(tool) {
            item.available
        } else {
            profile
                .runtime
