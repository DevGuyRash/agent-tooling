...
`packaging/sources/mermaid/upstream.json` captures the resolved official Git commit, recursively discovered syntax pages, full page contents and navigation metadata. `mermaid_refresh` fetches that snapshot explicitly. Both catalog tasks regenerate offline from those captured bytes. Failed retrieval, incomplete capture or renderer metadata failure preserves the accepted references.

Descriptions prefer upstream metadata, then explanatory introductory prose or blockquotes; early experimental notices remain separate source qualifications. Mermaid code fences supply documented starters. The bundled catalog observes their detection against the exact integrity-checked renderer, preserving ambiguous and unmatched entries. The broader catalog lists all captured documentation topics. Routes and source metadata supply documentation links; unpublished pages link to their pinned official source. Missing information stays visible. The catalog provides navigation for executor judgment; it does not prescribe diagram choices or counts.
